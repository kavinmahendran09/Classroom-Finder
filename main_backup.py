...
# Get the free rooms and print details
details = find_free_rooms()  # Custom time and Day_order example  : find_free_rooms("12:30","2")

# Check if details is a dictionary
if isinstance(details, dict):
    # Print details if it's a dictionary
    print(f"Today's Date: {details['current_date']}")
    print(f"Current Time: {details['current_time']}")
    print(f"Current Day Order: {details['current_day_order']}")
    print(f"Free classrooms right now are: {details['free_rooms']}")
else:
    # Print the error message if details is a string
    print(details)
